    ...
    Alert.alert("Excluir equipamento?", "Isso removerá o item permanentemente.", [
      { text: "Cancelar" },
      {
        ...
        onPress: async () => {
          ...
          arr = arr.filter((e: any) => String(e.id) !== String(id));
    ...
    Alert.alert("Remover 1 unidade?", undefined, [
      { text: "Cancelar" },
      {
        ...
        onPress: async () => {
          ...
          if (idx !== -1) {
            arr[idx].quantidade = Math.max(0, Number(arr[idx].quantidade) - 1);
            await AsyncStorage.setItem("equipamentos", JSON.stringify(arr));
            carregar();
          }
        }
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
          ...
          <View style={styles.card}>
            ...
            <View style={{ alignItems: "flex-end" }}>
              <TouchableOpacity
                style={[
                  styles.btn,
                  { backgroundColor: item.quantidade > 0 ? "#2B8AF6" : "#999" }
                ]}
                disabled={item.quantidade === 0}
                onPress={() =>
                  navigation.navigate("Aluguel", {
                    equipamentoId: String(item.id)
                  })
                }
              >
                <Text style={styles.btnTxtSmall}>
                  {item.quantidade === 0 ? "Indisponível" : "Aluguéis"}
                </Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                style={[styles.btn, { marginTop: 8, backgroundColor: "#E53935" }]}
                onPress={() => excluirEquipamento(item.id)}
              >
        ...
        ListEmptyComponent={() => (
          <Text style={{ color: "#666" }}>Nenhum equipamento cadastrado.</Text>
        )}
      />
    </View>
  );
